...
import { styled } from "@mui/material/styles";
import TableCell, { tableCellClasses } from "@mui/material/TableCell";
import { Button, Checkbox, Badge, CircularProgress } from "@mui/material";
import { debounce } from "lodash";
import { useSelector } from "react-redux";
import DefectedProductModal from "./DefectedProductModal";
...
const DefectedProduct = () => {
  ...
  const handlePdfDownload = (challanNumber) => {
    navigate(`/downloadchallanpdf/${challanNumber}`);
  };
  const handleSubmit = () => {
              ...
              data?.map((item, index) => {
                ...
                return (
                  <StyledTableRow key={index}>
                    ...
                    <StyledTableCell align="center">
                      {remark.map((log, i) => (
                        <p key={i} className="flex space-x-5 justify-center">
                          <span>Date:{log.date}</span>
                          <span>Remark:{log.remark}</span>
                        </p>
                      ))}
