...
const POTable = () => {
  ...
  const handleNavigate = () => {
    navigate("/po-form?poNumber=new&dealerId=" + params.dealerid);
  };
  const handleChangeRoute = (item) => {
    // console.log("poNumber ", item.poNumber)
    const encodedPoNumber = encodeURIComponent(item.poNumber);
    navigate(`/editpo?dealerId=${params.dealerid}&poNumber=${encodedPoNumber}`);
  };

  const handleSend = (item) => {
    const encodedPoNumber = encodeURIComponent(item.poNumber);
    navigate(
      `/sendproductdealer?dealerId=${params.dealerid}&poNumber=${encodedPoNumber}`,
      { state: item }
    );
  };
                ...
                return (
                  <StyledTableRow key={index}>
                    ...
                    <StyledTableCell align="center">
                      <Button
                        onClick={() => {
                          const url = `/pochallan/${params.dealerid}?poNumber=${item.poNumber}`;
